...
void ota_start_task (void *pvParameters){
	...
	if( ota_run() ){
		...
		ESP_LOGI(TAG, "OTA failed, continuing with other tasks..");
	}
...
//    ESP_LOGI(TAG, "Writing firmware... wait");
    ...
    while (flag) {
        ...
        if (buff_len < 0) { /*receive error*/
            ...
            flag = false;
            ESP_LOGI(TAG, "Connection closed, all packets received, total: %d bytes", binary_file_length);
            close(socket_id);
        } else {
